...
async function createToken(obj) {
    try {
        const myToken = jwt.sign({ user_name: user_name, role: role }, process.env.SECURET_TOKEN, { maxAge: 360000 })
...
async function verifyToken(myToken) {
    try {
        //a
        const paylode = jwt.decode(myToken)
        //b
...
export async function getTokenToCheck(req, res, next) {
    try {
        ...
        if (!myToken || myToken === undefined) {
            ...
            if (pass in req.headers && user_name in req.headers) {
